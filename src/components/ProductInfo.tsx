...
export function ProductInfo({ product }: { product: Product }) {
  return (
    <AnimatePresence mode="wait">
      <motion.div key={product.id} variants={containerVariants} initial="hidden" animate="visible" exit="exit">
        <motion.h2
          ...
          <motion.div variants={specsContainerVariants} className="flex items-center gap-3 flex-wrap max-w-sm">
            {product.specs.map((spec, index) => (
              <motion.span
                ...
                className={`py-2 px-5 rounded-full text-sm font-semibold ${index === 0 ? "border border-gray-300 text-gray-800" : "bg-gray-100 text-gray-800"}`}
              >
                {spec}
              </motion.span>
            ))}
